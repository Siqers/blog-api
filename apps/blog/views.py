...
class PostViewSet(viewsets.ModelViewSet):
    ...
    @extend_schema(
        ...
        **Caching**:
        ...
        - Cache dependency: The cache is automatically invalidated whenever a post is created, updated, or deleted.
        - Cache invalidation: When a post is created, updated, or deleted, the cache for all languages is cleared.

        **language behavior**:
        - The API supports multilingual content. The list of posts is cached separately for each language.
        - Date and time fields in the response are formatted according to the user's timezone and language preferences.

        **Timezone**
        - Authenticated users see date and time fields formatted according to their timezone preferences.
        - Anonymous users see date and time in UTC.

        **Authentication**: Both authenticated and anonymous users can access the list of posts.

        **Pagination**: The list of posts is paginated, with a default page size of 10 posts per page.
        """,
        parameters=[
            OpenApiParameter(
                name='lang',
    ...
    def list(self, request, *args, **kwargs):
        ...
        if page is not None:
            ...
            cache.set(cache_key, paginated_response.data, 60)
    ...
    @extend_schema(
        ...
        **Side effects**:
        ...
        - The cache for the list of posts is invalidated for all languages.
        - Rate limiting is applied. Each user can create a maximum of 20 posts per minute.
        """,
    ...
    @extend_schema(
        ...
        **Side effects:**
        ...
        - Dispatches a Celery task to create a notification and send WebSocket message
        """,
    ...
    def comments(self, request, slug=None):
        """
        GET /api/posts/{slug}/comments/ - list of comments
        ...
        if request.method == 'GET':
            ...
            if serializer.is_valid():
                ...
                logger.info('Comment added to post: %s  user %s', post.title, request.user.email)
